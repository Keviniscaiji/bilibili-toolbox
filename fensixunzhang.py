# -*- codeing = utf-8 -*-
# @Time : 2021/4/29 22:26
# @Author : Zhao Lingfei
# @File : fensixunzhang.py
# @Software : PyCharm
...
def getHTML(url, params = None):# 爬取网页，获取网页HTMl
    try:
        headers = {'user-agent':'chrome/83'} # 伪造请求头
        r = requests.get(url, params=params,headers=headers) #提交请求并获得响应

        r.encoding = 'utf-8' #b站直播采用utf-8编码
        r.raise_for_status   #如果状态码不为200即爬取失败及产生异常
...
with open('log.txt', 'w', encoding='utf-8') as logfile:  # 清空log.txt
    print('爬取开始',file = logfile)

with open('medal.txt','w', encoding='utf-8') as medalfile: # 清空medal.txt
...
for i in range(num1,num2):  # 遍历直播间
    ...
    if medalname: # 如果该直播间开通了粉丝勋章且粉丝榜有粉丝
        with open('medal.txt','a', encoding='utf-8') as medalfile: #以追加模式打开
            print('{}:{}'.format(medalname,i), file=medalfile) # 写入粉丝勋章及其对应的房间号

    if i% num_percent == 0: # 显示进度
        with open('log.txt','w',encoding='utf-8') as logfile:   # 以写入模式打开
            # 输出当前时间、进度，增强用户体验
            print('{} {}/{} {:2f}% complete'.format(time.ctime(), i-num1+1,num,i*100/num), file=logfile)
